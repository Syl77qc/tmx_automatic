...
Gestion des positions virtuelles mean reversion avec modèle de coûts réaliste.

Univers actif (7 FNBs) : XIU, XFN, XUT, XRE, XIN, XHC, XST
FNBs contextuels (5 FNBs) : XEG, ZAG, XGD, XIT, XMA — aucune position mean reversion

Changements v2.1 → v3.0 :
  - COMMISSION_CAD : 4.95 $ → 0.0 $ (Disnat — 0 commission FNBs CA en ligne)
  - BLOCS_CORRELES : bloc "metaux" retiré, ZAG retiré du bloc "taux"
  - FNBS_SENSIBLES_TAUX : ZAG retiré (indicateur contextuel)
  - PROFILS : horizons corrigés (Wilcoxon 25 ans), 5 FNBs contextuels marqués
  - calculer_taille_position : profil "lent" retiré (aucun FNB actif lent)
  - action_evaluer : garde-fou — FNBs contextuels rejetés si présents dans les signaux

Références PRD v3.0 :
  - Section 5    : Maillons 3 (filtres A-G) et 4 (exécution/suivi/sortie)
  - Section 6    : Univers 7 actifs + 5 contextuels
  - Section 7    : Dimensionnement des positions
  - Section 8    : Modèle de coûts (spread, 0 commission Disnat, slippage)
...
# Section 8 — Modèle de coûts (PRD v3.0 §8.1)
SPREAD_PCT        = 0.00050   # 0.05% par côté = 0.10% aller-retour
COMMISSION_CAD    = 0.0       # 0 $ — Disnat, FNBs canadiens achetés en ligne (PRD v3.0)
...
# Section 5, Maillon 3, Filtre G — Blocs corrélés (PRD v3.0)
# Bloc "metaux" retiré (XGD, XMA sont contextuels — aucune position mean reversion)
# ZAG retiré du bloc "taux" (contextuel en v3.0)
BLOCS_CORRELES = {
    "marche_large": {"XIU.TO", "XFN.TO", "XIN.TO"},
    "taux":         {"XRE.TO", "XUT.TO"},
}

# FNBs sensibles aux taux (filtre BdC RPM) — ZAG retiré (contextuel en v3.0)
FNBS_SENSIBLES_TAUX = {"XRE.TO", "XUT.TO", "XFN.TO"}

# ── Profils FNB (PRD v3.0 §6) ─────────────────────────────────────────────────
# Horizons horizon_j corrigés par Wilcoxon (25 ans, 2001-2026) :
#   XIU : 10 → 20  |  XFN : 15 → 20  |  XUT : 10 → 20 (marginal)
#   XRE : 25 → 20  |  XHC : 15 → 10  |  XST : 10 → 15  |  XIN : 10 ✓ confirmé
# Profil "lent" retiré — aucun FNB actif n'a ce profil en v3.0
...
PROFILS = {
    ...
    "XHC.TO": {"profil": "rapide", "seuil_min": 2.0, "horizon_j": 10,
               ...
               "bloc": None,           "actif": True},
    "XST.TO": {"profil": "rapide", "seuil_min": 2.0, "horizon_j": 15,
               "perte_additionnelle_moy": 0.031, "stop_loss_mult": 1.5,
               "bloc": None,           "actif": True},
    # ── FNBs contextuels — aucune position mean reversion ─────────────────────
    # z-scores calculés par le scanner, chocs alimentent contagion.py uniquement
    "XEG.TO": {"profil": None, "seuil_min": None, "horizon_j": None,
               "perte_additionnelle_moy": None, "stop_loss_mult": None,
               "bloc": None, "actif": False},
    "ZAG.TO": {"profil": None, "seuil_min": None, "horizon_j": None,
               "perte_additionnelle_moy": None, "stop_loss_mult": None,
               "bloc": "taux", "actif": False},
    "XGD.TO": {"profil": None, "seuil_min": None, "horizon_j": None,
               "perte_additionnelle_moy": None, "stop_loss_mult": None,
               "bloc": "metaux", "actif": False},
    "XIT.TO": {"profil": None, "seuil_min": None, "horizon_j": None,
               "perte_additionnelle_moy": None, "stop_loss_mult": None,
               "bloc": None, "actif": False},  # 87% baisses fondamentales (Shopify)
    "XMA.TO": {"profil": None, "seuil_min": None, "horizon_j": None,
               "perte_additionnelle_moy": None, "stop_loss_mult": None,
               "bloc": "metaux", "actif": False},
}
...
def filtre_B_regime(scan: dict) -> tuple[bool, str]:
    ...
    Risk-off → bloquer sauf signaux ≥ 3.0 é.-t. sur XST (seul défensif actif en v3.0).
    ZAG est contextuel — ne génère plus de signal mean reversion.
    """
...
def calculer_taille_position(
    ...
    # Base par profil (section 7.2) — profil "lent" retiré (aucun FNB actif lent en v3.0)
...
def action_evaluer(portefeuille: dict, trades_log: list, capital: float):
    ...
    for signal in scan["signaux"]:
        ...
        # Garde-fou v3.0 — FNBs contextuels ne génèrent pas de positions mean reversion
        profil_cfg = PROFILS.get(ticker, {})
        if not profil_cfg.get("actif", True):
            print(f"      ⏭  {ticker} est un FNB contextuel (v3.0) — aucune position mean reversion")
            continue
